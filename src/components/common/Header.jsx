      ...
      <header className="flex justify-center px-2">
        <div className="flex-1 flex items-center max-w-7xl py-3 px-2 lg:px-0">
          ...
          <div className="flex gap-5 ml-auto text-lg">
            {!isBooksPage && (
              <button onClick={() => setIsSearchOpen(true)} className="flex gap-2 hover:underline underline-offset-8">
            ...
            <Link to="/cart" className="flex gap-2 hover:underline underline-offset-8">
              <div className="relative">
                ...
                {cart.length > 0 && (
                  <span className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full text-xs px-1.5 py-0.5" data-testid="cart-count">
                    {cart.length}
                  </span>
                )}
              </div>
